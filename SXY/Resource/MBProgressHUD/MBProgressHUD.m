...
+ (instancetype)sharedHUD
{
    ...
    dispatch_once(&once, ^{
        _sharedHUD = [[self alloc] initWithView:[UIView new]];
//        _sharedHUD.margin = 12.f;
        _sharedHUD.labelFont = [UIFont systemFontOfSize:15];
        _sharedHUD.opacity = 0.8f;
//        _sharedHUD.animationType = MBProgressHUDAnimationZoom;
        _sharedHUD.mode = MBProgressHUDModeText;
    });
    
    return _sharedHUD;
}

+ (instancetype)shareLoadHUD
{
    static MBProgressHUD *_sharedLoadHUD = nil;
    
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        _sharedLoadHUD = [[self alloc] initWithView:[UIView new]];
        _sharedLoadHUD.opacity = 0.6f;
//        _sharedLoadHUD.dimBackground = YES;
        _sharedLoadHUD.labelFont = [UIFont systemFontOfSize:15];
    });
    
    return _sharedLoadHUD;
}
